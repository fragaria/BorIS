# -*- coding: utf-8 -*-
"""Step 2/3 of the practitioner migration - migrate data from the old columns to the new ones."""
import datetime
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        id_ = 0
        person_ptr_id_curr = 9999 # To be temporarily used for assigning person_ptr_id's.
        for p in list(orm['clients.Practitioner'].objects.all()):
            # For each practitioner create a new one for all his encounters (to be
            # converted to a PractitionerContact in a subsequent migration) and
            # delete the old one.
            encounters = orm['services.Encounter'].objects.filter(person=p.pk)
            for encounter in encounters:
                id_ += 1
                person_ptr_id_curr += 1
                pract_contact = orm['clients.Practitioner']()

                # First, fill in the non-used, but at this stage still required values.
                pract_contact.person_ptr_id = person_ptr_id_curr
                pract_contact.first_name, pract_contact.last_name = p.first_name, p.last_name
                pract_contact.organization, pract_contact.sex = p.organization, p.sex

                # And now the useful ones.
                pract_contact.id = id_
                pract_contact.person_or_institution = '%s%s%s' % (
                ...
                # clients_practitioner_users table right.
                pract_contact._pk = pract_contact.person_ptr_id
                pract_contact.person_ptr_id = pract_contact.id
                for user in encounter.performed_by.all():
                    pract_contact.users.add(user.id)
                pract_contact.person_ptr_id = pract_contact._pk

                pract_contact.save()

                orm['services.Service'].objects.filter(encounter=encounter.pk).delete()
                encounter.delete()
                # Deleting the encounters and services from services migrations
    ...
    models = {
        ...
        'clients.practitioner': {
            ...
            'id': ('django.db.models.fields.PositiveIntegerField', [], {'null': 'True'}),
            'note': ('django.db.models.fields.TextField', [], {'blank': 'True'}),
            'person_or_institution': ('django.db.models.fields.CharField', [], {'max_length': '255', 'null': 'True'}),
            'town': ('django.db.models.fields.related.ForeignKey', [], {'related_name': "'+'", 'to': "orm['clients.Town']"}),
            'users': ('django.db.models.fields.related.ManyToManyField', [], {'to': "orm['auth.User']", 'symmetrical': 'False'}),

            # Keep the old fields until the migration is done (i.e. 0008).
            'first_name': ('django.db.models.fields.CharField', [], {'max_length': '63', 'null': 'True', 'blank': 'True'}),
