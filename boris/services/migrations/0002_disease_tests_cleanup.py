...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        "Delete services that do not correspond to the new set up."
        # Delete DiseaseTests where no test has been performed.
        orm.DiseaseTest.objects.filter(disease__isnull=True).delete()
        orm.DiseaseTest.objects.filter(sign__isnull=True).delete()
        orm.DiseaseTest.objects.filter(sign='').delete()

        # Delete UtilityWork instances where only some of the two removed refs
        # are present.
        for uw in orm.UtilityWork.objects.all():
            if uw.refs and set(uw.refs).issubset(set(['no', 'crc'])):
                uw.delete()
