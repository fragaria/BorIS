...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        try:
            ct_filter = {'app_label': 'services', 'model': 'PhoneCounseling'}
            phone_ct = orm['contenttypes.ContentType'].objects.filter(**ct_filter)[0]
        except IndexError:
            return # Freshly setup database - nothing more to do.

        for phone_counseling in orm['services.Service'].objects.filter(content_type=phone_ct):
            encounter = phone_counseling.encounter
            encounter.is_by_phone = True
            encounter.save()
        orm['services.Service'].objects.filter(content_type=phone_ct).delete()
        phone_ct.delete()

        codenames = ('add_phonecounseling', 'change_phonecounseling', 'delete_phonecounseling')
        orm['auth.Permission'].objects.filter(codename__in=codenames).delete()
