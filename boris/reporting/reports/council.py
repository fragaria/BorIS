...
class GovCouncilReport(BaseReport):
    ...
    def _get_data_services(self):
        ...
        return [
            (_(u'Osobní kontakt s klienty'), directly_encountered_clients_count, direct_client_encounters.count()),
            (_(u'– z toho prvních kontaktů'), clients(IncomeExamination), services(IncomeExamination)),
            (_(u'Úkony potřebné pro zajištění práce s klientem'), 'xxx', services(Address)),
            (_(u'Kontaktní práce'), clients(ContactWork) + anon(ContactWork), services(ContactWork)),
            (_(u'Vstupní zhodnocení stavu klienta'), clients(IncomeFormFillup), services(IncomeFormFillup)),
            (_(u'Individuální poradenství'), clients(IndividualCounseling), services(IndividualCounseling)),
            (_(u'Individuální psychoterapie'), 0, 0),
            (_(u'Skupinové poradenství'), 0, 0),
            (_(u'Skupinová psychoterapie'), 0, 0),
            (_(u'Krizová intervence'), clients(CrisisIntervention), services(CrisisIntervention)),
            (_(u'Rodinná terapie'), 0, 0),
            (_(u'Skupiny pro rodiče a osoby blízké klientovi'), 0, 0),
            (_(u'Pracovní terapie'), 0, 0),
            (_(u'Sociální práce (odkazy, asistence, soc.-právní pomoc, case management)'), clients(SocialWork), services(SocialWork)),
            ...
            (_(u'Základní zdravotní ošetření (vč. první pomoci, volání ZS)'), clients(BasicMedicalTreatment), services(BasicMedicalTreatment)),
            (_(u'Telefonické, písemné a internetové poradenství'), 'xxx', self._get_phone_advice_count()),
            (_(u'Korespondenční práce'), 0, 0),
            (_(u'Informační servis'), clients(InformationService), services(InformationService) - anon(InformationService)),
            (_(u'Edukativní program/beseda'), 0, 0),
            (_(u'Výměnný program'), clients(HarmReduction), services(HarmReduction)),
            (_(u'– vydané injekční jehly'), 'xxx', harm_reductions.aggregate(Sum('out_count'))['out_count__sum']),
            (_(u'– přijaté injekční jehly'), 'xxx', harm_reductions.aggregate(Sum('in_count'))['in_count__sum']),
            (_(u'– nalezené injekční jehly'), 'xxx', self._get_syringes_count()),
            (_(u'Hygienický servis'), 0, 0),
            (_(u'Potravinový servis'), 0, 0),
            (_(u'Testování na inf. nemoci'), self._get_tested_clients_count(), self._get_performed_tests_count()),
            (_(u'– z toho na HIV'), self._get_tested_clients_count(DISEASES.HIV), self._get_performed_tests_count(DISEASES.HIV)),
            (_(u'– z toho na HCV'), self._get_tested_clients_count(DISEASES.VHC), self._get_performed_tests_count(DISEASES.VHC)),
            (_(u'– z toho na HBV'), self._get_tested_clients_count(DISEASES.VHB), self._get_performed_tests_count(DISEASES.VHB)),
            (_(u'– z toho na syfilis'), self._get_tested_clients_count(DISEASES.SYFILIS), self._get_performed_tests_count(DISEASES.SYFILIS)),
